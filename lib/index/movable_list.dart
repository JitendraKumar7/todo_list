...
typedef bool MoverItemCallback(Key draggedItem, Key newPosition);
typedef void MoverCompleteCallback(Key draggedItem);
...
class MovableList extends StatefulWidget {
  ...
  final MoverItemCallback onMover;
  final MoverCompleteCallback? onMoverDone;
  final DecoratePlaceholder decoratePlaceholder;

  final CancellationToken? cancellationToken;

  @override
  State<StatefulWidget> createState() => _MovableListState();
}
...
typedef Widget MovableItemChildBuilder(
    BuildContext context,
    MovableItemState state,
    );
...
class MovableItem extends StatefulWidget {
  ...
  final MovableItemChildBuilder childBuilder;

  @override
  createState() => _MovableItemState();
}

typedef MovableListenerCallback = bool Function();

class MovableListener extends StatelessWidget {
  MovableListener({
    Key? key,
    this.child,
    this.canStart,
  ...
  final MovableListenerCallback? canStart;
  ...
  void _startDragging({required BuildContext context, PointerEvent? event}) {
    _MovableItemState? state =
    context.findAncestorStateOfType<_MovableItemState>();

    final scrollable = Scrollable.of(context);

    final listState = _MovableListState.of(context)!;
...
class DelayedMovableListener extends MovableListener {
  DelayedMovableListener({
    Key? key,
    Widget? child,
    MovableListenerCallback? canStart,
...
class _MovableListState extends State<MovableList>
  ...
  void _cancel() {
    if (_dragging != null) {
      ...
      if (widget.onMoverDone != null) {
        widget.onMoverDone!(dragging);
      }
    }
  }
  ...
  Drag _dragStart(Offset position) {
    ...
    _dragProxy!.setWidget(
        draggedItem.widget
            .childBuilder(draggedItem.context, MovableItemState.dragProxy),
  ...
  void cancel() {
    ...
    _dragProxy!.updateWidget(current.widget
        .childBuilder(current.context, MovableItemState.dragProxyFinished));
  ...
  void checkDragPosition() {
    ...
    List<Function> onMoverApproved = [];

    if (_dragProxy!.offset < draggingTop) {
      for (_MovableItemState item in _items.values) {
    ...
    if (closest != null &&
      ...
      if (widget.onMover(_dragging!, closest.key)) {
        bool isIOS = Theme.of(context).platform == TargetPlatform.iOS;

        if (isIOS) {
          _hapticFeedback();
        }
        for (final f in onMoverApproved) {
          f();
        }
        _lastReportedKey = null;
      }
  ...
  final HashMap<Key?, _MovableItemState> _items =
  HashMap<Key, _MovableItemState>();

  void registerItem(_MovableItemState item) {
    _items[item.key] = item;
  }

  void unregisterItem(_MovableItemState item) {
    if (_items[item.key] == item) _items.remove(item.key);
  }

  double _itemOffset(_MovableItemState item) {
  ...
  static _MovableListState? of(BuildContext context) {
    return context.findAncestorStateOfType<_MovableListState>();
  }
...
class _MovableItemState extends State<MovableItem> {
  ...
  Widget build(BuildContext context) {
    // super.build(context);
    _listState = _MovableListState.of(context);
    ...
    return Transform(
      ...
      child: widget.childBuilder(
          context,
          dragging
              ? MovableItemState.placeholder
              : MovableItemState.normal),
    );
  }
...
class _DragProxyState extends State<_DragProxy> {
  ...
  void setWidget(Widget widget, RenderBox position) {
    setState(() {
      ...
      _widget = widget;
      final state = _MovableListState.of(context)!;
  ...
  @override
  void deactivate() {
    _MovableListState.of(context)?._dragProxy = null;
    super.deactivate();
  }
}
